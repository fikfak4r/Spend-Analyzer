...
namespace SpendAnalyzer.Administration.Repositories
{
    using Serenity;
    ...
    public partial class UserRepository
    {
        ...
        private class MySaveHandler : SaveRequestHandler<MyRow>
        {
            ...
            {
                if (!String.IsNullOrEmpty(Row.Tenants))
                {

                    Connection.Execute(String.Format("DELETE FROM Tenants WHERE UserId = {0}", Row.UserId));

                    String[] tenantIds = Row.Tenants.Split(',');

                    for (int x = 0; x < tenantIds.Length; x++)
                    {
                        Connection.Execute(String.Format("INSERT INTO Tenants (UserId, TenantId) VALUES({0}, {1})", Row.UserId, tenantIds[x]));
                    }
